...
export function uploadFile(file, dirId) {
    return async dispatch => {
        try {
            ...
            const uploadedFile = { name: file.name, size: file.size, progress: 0, id: Date.now() + file.name};
            ...
            const response = await axios.post(`${API_URL}api/files/upload`, formData, {
                ...
                onUploadProgress: progressEvent => {
                    uploadedFile.progress = Math.round((progressEvent.loaded / progressEvent.total) * 100)
